...
SesameScanner::onResult(NimBLEAdvertisedDevice* adv) {
	...
	if (manu_data.length() < 5 || manu_data[0] != 0x5a || manu_data[1] != 0x05) {
		DEBUG_PRINTF("%s: Unexpected manufacturer data, ignore this device\n", addr.toString().c_str());
		return;
	}
	...
	if (model == Sesame::model_t::wifi_2) {
		...
		if (name.length() != 22) {
			DEBUG_PRINTF("%u: %s: Unexpected name field length, ignore this device\n", manu_data[2], addr.toString().c_str());
			return;
		}
		...
		if (rc != 0 || idlen != sizeof(uuid_bin)) {
			DEBUG_PRINTF("%s: Unexpected name format, ignore this device\n", addr.toString().c_str());
			return;
		}
	}
